...
| a wall of people / a roster | §Surfaces (the Wall), Recipe E |
...
This runs on a single near-black canvas — there is **no light theme and no light, airy editorial**. The photographs do the talking: a screen can become a **wall** of human moments, tiled and captioned, where the pictures are the only color and the chrome all but disappears (the grid is gaps of near-black). A roster isn't a table of records; it's faces you'd know by sight.
...
6. **People are required.** Photography is recurring and load-bearing, not optional — one human moment per section, or a whole **wall** of them. Every product section ships with its paired face; full-color, untouched, film-grain on top.
...
```css
:root {
  /* surfaces — the only canvas is near-black */
...
> No `[data-theme]`. Stadli is dark-ground only — there is no light parity theme, no light editorial register. Every band, including the wall, sits on near-black.
...
- **Meta = the forensic voice:** file-paths, `updated 12s ago`, `n = 18,442 · source: ticketing`, `SEC 104 · R12 · S07`. It annotates, never titles; it captions a photo on the wall but never the manifesto or the inside of a quote.
...
- **Photography is the loudest color.** On a wall or a full-bleed band, the photographs supply all the color; the surrounding chrome shrinks to captions and hairlines. Color in the picture is unlimited; color in the UI is still none.
...
- **`wide`** = the default (dashboards, prose, forms, tables). **`full`** = immersion (hero, photo, atmosphere, the wall, the Moment); its text re-wraps in a `.wide`. **`bleed`** = a headline opting a fixed amount wider; never body, chrome, or meta.
- `.wide` and `.bleed` are both centered and share `--edge`, so they line up automatically — nothing to keep in sync.
- **Rhythm:** read the page like a magazine cut into bands — black product band → full-bleed photo / atmosphere / **wall** → black. One idea per band. 5–9 widgets per product view. Never 3+ product sections without a photo, wall, or atmosphere break. One manifesto per page.

---

## §Surfaces — texture, photography, the Wall, the Moment
...
**Photography (Law 6).** Required and recurring — a marketing page runs ≥3 full-bleed photo bands; a product page shows a face every ~2 screens. Faces (mid-emotion, not posed) → crowds → architecture → operational. The only treatment is a fine colorless **film grain (~6%)** — no duotone, tint, grayscale, or overlay. For color + photo, layer a solid block on top; copy on it stays monochrome. Full-bleed bands and wall tiles may carry **muted, looping media** (clips, not just stills — the Runway/Krea register) under the identical rules: grain on top, no tint, and a freeze on `prefers-reduced-motion`. **Forbidden:** laptops-showing-product, conference rooms, handshakes, stock headshots, drone shots, empty seats, posed "diverse team," lens flares.

**The Wall (gallery / contact sheet).** The favorites' move — Cosmos, Savee, Are.na, Krea, DICE — but on black. A curated grid of real people: faces first, then crowds, then moments, each tile a full-color untouched photo with grain, each captioned in meta (`name · SEC 104 · 2026`). The gaps between tiles are near-black canvas (the whitespace); the grid chrome is hairlines and nothing else. The photographs carry every bit of color; type shrinks to captions. This is *people are what's in color* rendered at scale — a screen that is almost entirely photography on black. Still Law 2: at most **one** colored data dot on the whole wall, and only if it means something. **Never a light gallery** — the wall sits on near-black like every other band.
...
Exposed at `window.StadliDesignSystem`. The point of using them is that the wrong thing is *impossible*: a pill can't colorize its label, a card can't grow a shadow, a chart can't run two story-colors, a wall tile can't be tinted.
...
**People**
- `Wall` — the curated grid (§Surfaces); product *and* marketing (a roster, an opening-night recap). Faces fill it, captions ride in meta, gaps are canvas, photos untouched + grain. `items:[{image|media, alt, caption}] · columns · density: tight|airy`.
...
**Marketing bands** (`MarketingBands`) — text always monochrome; color lands on a dot, an `.atmo-*` field, the photo, or a block. `SectionManifesto · StatBand · DotMatrixBand · PeopleBand · Wall · QuoteBand · DissolvePortrait · CollageStrip`.
...
**E — the wall** (people are the only color; near-black gaps)
```jsx
<section className="full">
  <Wall density="tight" columns={4} items={[
    { image:"https://stadli.com/assets/photography/crowd-reach.png", alt:"hands up, back row",   caption:"SEC 104 · opening night" },
    { image:"https://stadli.com/assets/photography/eye-closeup.png", alt:"a fan, mid-roar",       caption:"R12 · S07 · 2026" },
    { media:"https://stadli.com/assets/photography/crowd-dancer.jpg", alt:"the wave breaking",    caption:"the wave · 9:41 PM" },
    { image:"https://stadli.com/assets/photography/foam-finger.png", alt:"thirty-year season-ticket holder", caption:"GATE B · since 1994" },
    /* …more real people, each a real caption; one optional --ok/--info data dot for the whole wall, max… */
  ]} />
</section>
```
...
- **Curatorial, with a point of view.** Write like a publication that knows its scene — the register of a good liner note, not a tool's tooltip. A caption can have a stance; a section can argue one thing and mean it.
...
- [ ] Tint/duotone/overlay on a photo; photography missing; a mockup without its paired face; a wall that isn't real people.
- [ ] A data dot with no citation; a 5th chart color (use dashed/weight/point); dual y-axis or truncated bar baseline.
- [ ] A light theme, or any light/airy editorial band — Stadli is near-black only; the wall and every band sit on the dark ground.
- [ ] Fake scarcity or dark patterns.

---

## §Assets — hosted URLs

Use directly in `image`/`src`. Grain still applies; §Surfaces photo + wall rules hold.
...
**In one line:** monochrome by contract, near-black only; *people are what's in color when they show up* — one face, or a wall of them. Two volumes, never confused — the work is calm, the Moment is loud and earned. Type, the tabular number, the dot, and the human face carry every meaning; color is light, rationed to atmosphere.
