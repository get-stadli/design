...
colors:
  ...
  spec-green: "#22c55e"        # positive · success · growth  +  benchmark · active   (v9 green + teal)
...
| **Green** | `spec-green` | `#22c55e` | Positive growth · target met · goal achieved — **and** benchmark · baseline · comparison anchor | Success · confirmed · resolved · the upward delta — **and** active · selected · toggled-on | Momentum · wins · the go-signal |
...
- **The health-monitor carve-out.** The one place a pill's dot may wear `spec-green` for a *standing* positive state is a live health or uptime monitor, where « opérationnel » is a measured pass rather than an idle label (see "Exception-first — the color budget"). Everywhere else green stays reserved for resolution and growth, never the expected resting state.
...
- **One story series.** The series carrying the story takes the spec color whose data-viz meaning matches it (azure for the headline metric, purple for the forecast, green for the benchmark…). Comparison and context series stay monochrome — `muted` and `muted-soft` strokes. Prefer one colored series; never exceed three spec colors in a single chart.
